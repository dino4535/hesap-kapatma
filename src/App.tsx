...
export default function App() {
  ...
  const bayiHavaleEslemeRows = useMemo(() => {
    return havaleVadeliByBayiRows.map((r) => {
      ...
      const gelenBugun = manimIncomingNotTransferredTodayByCorrespondentCode.get(matchCode) ?? 0
    ...
    havaleVadeliByBayiRows,
    manimIncomingNotTransferredTodayByCorrespondentCode,
  ...
  const printMutabakatPdf = async () => {
    ...
    const havaleEslemeRows = havaleVadeliRows.map((r) => {
      const matchCode = normalizeMatchCode(r.bayiKodu)
      const gelenBugun = incomingNotTransferredTodayByCorrespondentForPrint.get(matchCode) ?? 0
