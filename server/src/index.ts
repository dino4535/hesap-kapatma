...
async function fetchCariGunAlacakToplamlari(args: { dayYmd: string; cariCodes: string[] }) {
  ...
  const req = pool.request()
  req.input('Day', mssql.Date, args.dayYmd)
  const inParams: string[] = []
  values.forEach((v, idx) => {
    const name = `cdc${idx}`
    inParams.push(`@${name}`)
...
app.post('/api/cari-day-credits', async (req, res) => {
  try {
    const userName = String(req.header('x-user') ?? '').trim()
    if (!userName) {
      res.status(401).send('Yetkisiz')
    ...
    const parsedDay = parseYmdDateText(req.body?.day)
    if (!parsedDay.ok) {
      res.status(400).send(parsedDay.error)
      return
    }
